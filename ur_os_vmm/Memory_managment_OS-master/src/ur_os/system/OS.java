...
public class OS {
    ...
    Random r;
    boolean lazySwap;
    private FreeMemoryManager slotManager;
    public static int totalInternalFragmentation = 0;
    ...
    public void removeProcessFromCPU(){
        cpu.removeProcess();
        // --------------------------nuevo código-------------------------------
        printTotalPagingInternalFragmentation();
        printExternalFragmentationStats();
        // ---------------------------------------------------------------------
    }
    ...
    public static int calculateInternalFragmentation(List<Process> processes) {
        int totalFragmentation = 0;

        for (Process process : processes) {
            if(process.getState() != ProcessState.FINISHED){
                int memoryRequired = process.getSize(); // Ajusta si el atributo tiene otro nombre
                int pagesNeeded = (int) Math.ceil((double) memoryRequired / PAGE_SIZE);
                int allocatedMemory = pagesNeeded * PAGE_SIZE;

                totalFragmentation += (allocatedMemory - memoryRequired);
            }
        }
    ...
    public void printTotalPagingInternalFragmentation() {
        int total = this.calculateInternalFragmentation(rq.getAllProcesses());
        System.out.println("Fragmentación interna total (paging): " + total + " unidades");
    }

    
    public double calculateExternalFragmentation(List<Integer> freeMemoryBlocks) {
        if (freeMemoryBlocks == null || freeMemoryBlocks.isEmpty()) {
            return 0;
        }

        if (freeMemoryBlocks.size() == 1) {
            return 0;
        }

        // Ordenar los bloques de menor a mayor
        List<Integer> sortedBlocks = new ArrayList<>(freeMemoryBlocks);
        Collections.sort(sortedBlocks);

        // Excluir el bloque más grande
        List<Integer> blocksToConsider = sortedBlocks.subList(0, sortedBlocks.size() - 1);

        int sum = 0;
        for (int block : blocksToConsider) {
            sum += block;
        }

        double average = (double) sum / blocksToConsider.size();
    ...
    public void printExternalFragmentationStats() {
        if (slotManager != null && slotManager instanceof FreeMemorySlotManager) {
            ur_os.memory.freememorymagament.FreeMemorySlotManager sm =
                (FreeMemorySlotManager) slotManager;

            List<MemorySlot> slots = new ArrayList<>(sm.getSlots());
            List<Integer> blockSizes = new ArrayList<>();

            for (MemorySlot slot : slots) {
                blockSizes.add(slot.getSize());
            }

            double fragmentation = calculateExternalFragmentation(blockSizes);
            System.out.println("Fragmentación externa: " + fragmentation + " unidades");
        }
    }
    // -------------------------------------------------------------------------
